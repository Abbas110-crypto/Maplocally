...
const PostRow = ({ posts }) => {
  ...
  const scrollLeft = () => {
    if (rowRef.current) {
      rowRef.current.scrollBy({ left: -320, behavior: 'smooth' }); // Scroll left by 300 pixels
    }
  };

  const scrollRight = () => {
    if (rowRef.current) {
      rowRef.current.scrollBy({ left: 320, behavior: 'smooth' }); // Scroll right by 300 pixels
    }
  };
  ...
  return (
    <div className={styles.cardRowContainer}>
      ...
      {isScrollable && rowRef.current && scrollPos < (rowRef.current.scrollWidth - rowRef.current.clientWidth) && (
